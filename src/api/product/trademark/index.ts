// 品牌管理模块接口
import request from '@/utils/request'

import type { TradeMarkResponseData, TradeMark } from './type'

// 品牌管理模块接口地址
enum API {
  // 获取已有品牌接口
  TRADEMARK_API = '/admin/product/baseTrademark/',
  // 添加品牌接口
  ADD_TRADEMARK_API = '/admin/product/baseTrademark/save',
  // 修改已有品牌接口
  UPDATE_TRADEMARK_API = '/admin/product/baseTrademark/update',
  // 删除已有品牌
  DELETE_TRADEMARK_URL = '/admin/product/baseTrademark/remove/',
}

// 获取已有品牌的接口方法
// 携带参数: page / limit
export const reqTrademark = (page: number, limit: number) => request.get<any, TradeMarkResponseData>(API.TRADEMARK_API + `${page}/${limit}`)

// 添加与修改已有品牌接口方法
export const reqAddOrUpdateTrademark = (data: TradeMark) => {
  if (data.id) {
    // 修改品牌
    return request.put<any, any>(API.UPDATE_TRADEMARK_API, data)
  } else {
    // 新增品牌
    return request.post<any, any>(API.ADD_TRADEMARK_API, data)
  }
}

// 删除已有品牌的接口方法
export const reqDeleteTrademark = (id: number) => request.delete<any, any>(API.DELETE_TRADEMARK_URL + id)
